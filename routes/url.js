...
router.get('/:short_url/search', function (req, res) {
    return findUrlByShortUrl(req.params.short_url).then(
        response => res.status(200).send(response),
        error => res.status(404).send('Error getting urls')
    );
});
...
router.get('/:short_url/edit', function (req, res) {
    return res
        .redirect(301, `${prefix}edit/?short_url=${req.params.short_url}`)
        .then(
            response => res.status(200).send(response),
            error => res.status(404).send('Error getting urls')
        );
    ...
    findByShortUrlAndUpdate(urlPair)
        .then(
            ...
            function (error) {
                return res
                    .status(500)
                    .send('Issue updating long url by short url');
            }
        )
        .then(function () {
...
router.delete('/:short_url', function (req, res) {
    return deleteByShortUrl(req.params.short_url).then(
        response => res.status(200).send(response),
